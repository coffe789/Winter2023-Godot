...
func _process(_delta):
	...
	if not is_in_group("dead_kitty") and not flag.is_in_group("dead_flag") and not Globals.disable_walk:
		if Input.is_action_just_pressed("ui_right"):
			flag.add_history()
			if dragon: dragon.add_history()
			add_history()
			level.send_in_direction(Vector2.RIGHT, flag)
			if dragon: level.send_in_direction(Vector2.LEFT, dragon)
			...
			if dragon: level.send_in_direction(Vector2.DOWN, dragon)
			...
			if dragon: level.send_in_direction(Vector2.RIGHT, dragon)
			...
			if dragon: level.send_in_direction(Vector2.UP, dragon)
