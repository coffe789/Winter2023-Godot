...
func _process(_delta):
	...
	if not is_in_group("dead_kitty") and not flag.is_in_group("dead_flag") and not Globals.disable_walk:
		if Input.is_action_just_pressed("ui_right"):
			...
			if dragons: for dragon in dragons: level.send_in_direction(Vector2.DOWN, dragon)
			do_turn()
			...
			if dragons: for dragon in dragons: level.send_in_direction(Vector2.RIGHT, dragon)
			do_turn()
			...
			if dragons: for dragon in dragons: level.send_in_direction(Vector2.UP, dragon)
			do_turn()
